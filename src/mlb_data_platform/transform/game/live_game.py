"""Complete Game.liveGameV1() transformation.

Uses the extraction registry pattern from base.py.
Subclass only needs to register extractions, not implement __call__().
...
Example usage:
    ...
    >>> transform = LiveGameTransform(spark)
...
class LiveGameTransform(BaseTransformation):
    """Game transformation using extraction registry pattern.

    Transforms raw game.live_game_v1 JSONB data into 16 normalized tables:
    ...
    **Player Lists (5 tables):**
    - live_game_players
    - live_game_batting_order (both teams, consolidated)
    - live_game_bench (both teams, consolidated)
    - live_game_bullpen (both teams, consolidated)
    - live_game_pitchers (both teams, consolidated)

    **Scoring (1 table):**
    - live_game_linescore_innings

    **Play-by-Play (7 tables):**
    - live_game_plays
    - live_game_play_actions
    - live_game_pitch_events
    - live_game_hit_data (Statcast batted ball data, linked via play_id)
    - live_game_runners
    ...
    def __init__(
        ...
        # Group 2: Player Lists (5 tables) - consolidated home/away
        self.register_extraction(
            name="players",
        ...
        self.register_extraction(
            name="batting_order",
            method=self._extract_batting_order,
            target_table="game.live_game_batting_order",
        )
        self.register_extraction(
            name="bench",
            method=self._extract_bench,
            target_table="game.live_game_bench",
        )
        self.register_extraction(
            name="bullpen",
            method=self._extract_bullpen,
            target_table="game.live_game_bullpen",
        )
        self.register_extraction(
            name="pitchers",
            method=self._extract_pitchers,
            target_table="game.live_game_pitchers",
        )
        ...
        self.register_extraction(
            name="hit_data",
            method=self._extract_hit_data,
            target_table="game.live_game_hit_data",
        )
        self.register_extraction(
            name="runners",
    ...
    def _extract_batting_order(self, raw_df: DataFrame) -> DataFrame:
        """Extract batting order (1-9) for both teams (consolidated).

        Args:
            raw_df: Raw game data

        Returns:
            DataFrame with columns: game_pk, team_id, is_home, batting_position, player_id
        """
        schema = StructType([
            StructField("gamePk", IntegerType(), True),
            StructField("gameData", StructType([
        ...
        # Extract home batting order
        home_batters_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.home.id").alias("team_id"),
            F.posexplode(F.col("parsed.liveData.boxscore.teams.home.batters")).alias(
                "position", "player_id"
            ),
        ).filter(F.col("position") < 9).select(
            F.col("game_pk"),
            F.col("team_id"),
        ...
        # Extract away batting order
        away_batters_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.away.id").alias("team_id"),
            F.posexplode(F.col("parsed.liveData.boxscore.teams.away.batters")).alias(
                "position", "player_id"
            ),
        ).filter(F.col("position") < 9).select(
            F.col("game_pk"),
        ...
        # Union home and away
        return home_batters_df.union(away_batters_df)

    def _extract_bench(self, raw_df: DataFrame) -> DataFrame:
        """Extract bench players for both teams (consolidated).

        Args:
            raw_df: Raw game data

        Returns:
            DataFrame with columns: game_pk, team_id, is_home, player_id
        """
        schema = StructType([
            StructField("gamePk", IntegerType(), True),
            StructField("gameData", StructType([
        ...
        # Extract home bench
        home_bench_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.home.id").alias("team_id"),
            F.explode(F.col("parsed.liveData.boxscore.teams.home.bench")).alias("player_id"),
        ).select(
            F.col("game_pk"),
            F.col("team_id"),
            F.lit(True).alias("is_home"),
        ...
        # Extract away bench
        away_bench_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.away.id").alias("team_id"),
            F.explode(F.col("parsed.liveData.boxscore.teams.away.bench")).alias("player_id"),
        ).select(
            F.col("game_pk"),
            F.col("team_id"),
            F.lit(False).alias("is_home"),
        ...
        # Union home and away
        return home_bench_df.union(away_bench_df)

    def _extract_bullpen(self, raw_df: DataFrame) -> DataFrame:
        """Extract bullpen pitchers for both teams (consolidated).

        Args:
            raw_df: Raw game data

        Returns:
            DataFrame with columns: game_pk, team_id, is_home, pitcher_id
        """
        schema = StructType([
            StructField("gamePk", IntegerType(), True),
            StructField("gameData", StructType([
        ...
        # Extract home bullpen
        home_bullpen_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.home.id").alias("team_id"),
            F.explode(F.col("parsed.liveData.boxscore.teams.home.bullpen")).alias("pitcher_id"),
        ).select(
            F.col("game_pk"),
            F.col("team_id"),
            F.lit(True).alias("is_home"),
        ...
        # Extract away bullpen
        away_bullpen_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.away.id").alias("team_id"),
            F.explode(F.col("parsed.liveData.boxscore.teams.away.bullpen")).alias("pitcher_id"),
        ).select(
            F.col("game_pk"),
            F.col("team_id"),
            F.lit(False).alias("is_home"),
        ...
        # Union home and away
        return home_bullpen_df.union(away_bullpen_df)

    def _extract_pitchers(self, raw_df: DataFrame) -> DataFrame:
        """Extract all pitchers for both teams (consolidated).
        ...
        home_pitchers_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.home.id").alias("team_id"),
            F.explode(F.col("parsed.liveData.boxscore.teams.home.pitchers")).alias("pitcher_id"),
        ).select(
            F.col("game_pk"),
            F.col("team_id"),
            F.lit(True).alias("is_home"),
        ...
        away_pitchers_df = parsed_df.select(
            F.col("parsed.gamePk").alias("game_pk"),
            F.col("parsed.gameData.teams.away.id").alias("team_id"),
            F.explode(F.col("parsed.liveData.boxscore.teams.away.pitchers")).alias("pitcher_id"),
        ).select(
            F.col("game_pk"),
            F.col("team_id"),
            F.lit(False).alias("is_home"),
        ...
        # Union home and away
        return home_pitchers_df.union(away_pitchers_df)
    ...
    def _extract_pitch_events(self, raw_df: DataFrame) -> DataFrame:
        """Extract every pitch thrown with Statcast pitch tracking data.

        Includes pitchData: velocity, spin rate, movement, location, release point.
        Hit data (exit velo, launch angle) is in separate hit_data table linked via play_id.
        ...
        Returns:
            DataFrame with columns: game_pk, play_id, pitch_number,
                pitch_type, start_speed, end_speed, spin_rate, zone, coordinates, etc.
        """
    ...
    def _extract_hit_data(self, raw_df: DataFrame) -> DataFrame:
        """Extract Statcast batted ball data (only when ball is put in play).

        This is a separate table from pitch_events because:
        1. Only ~15% of pitches result in balls in play
        2. Hit data is a distinct analytical domain (batted ball vs pitch mechanics)
        3. Enables efficient Statcast barrel/hard-hit queries
        ...
        Returns:
            DataFrame with columns: game_pk, play_id, at_bat_index,
        ...
        # Explode play events and filter to pitches that are in play
        hit_events_df = plays_df.select(
            F.col("game_pk"),
            F.col("play.about.atBatIndex").alias("at_bat_index"),
            F.explode(F.col("play.playEvents")).alias("event")
        ).filter(
            (F.col("event.isPitch") == True) &
            (F.col("event.details.isInPlay") == True) &
            (F.col("event.hitData.launchSpeed").isNotNull())
        )

        # Extract hit data fields
        hit_data_df = hit_events_df.select(
            F.col("game_pk"),
            F.col("event.playId").alias("play_id"),
            F.col("at_bat_index"),
            ...
            F.col("event.hitData.location").cast(IntegerType()).alias("hit_location"),
